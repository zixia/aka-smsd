...
int prepf(int fp,struct pattern_image** ppatt_img,size_t* patt_image_len)  //读取pattern文件
{
...
int mgrep_str(char *text, int num,struct pattern_image* patt_img)  //匹配字符串
{
    if (patt_img->SHORT)
...
static countline(text, len)				//计算文章行数
unsigned char *text;
int len;
{
    int i;
